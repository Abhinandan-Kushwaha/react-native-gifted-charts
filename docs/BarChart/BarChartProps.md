...
| onScrollEndDrag                     | (event,direction) => void             | callback function called when the chart is has finished scrolling. NOTE: the `direction` can be `-1`(indicating scroll end to left hand side) or `1`(indicating scrol end to right hand side). You can customise the `endReachedOffset` to determine the offset from "end"                                                                                  | \_                  |
